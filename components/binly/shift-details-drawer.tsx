                  ...
                  {(() => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* Task Type Icon */}
                        <div className="flex-shrink-0">
                          <div className={`w-7 h-7 rounded-md flex items-center justify-center ${redeploy ? 'bg-teal-50' : getTaskBgColor(task.task_type)}`}>
                            <TaskIcon className={`w-3.5 h-3.5 ${redeploy ? 'text-teal-600' : getTaskColor(task.task_type)}`} />
                          </div>
                        </div>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5">
                            <p className={`text-xs font-medium truncate ${isCompleted || isSkipped ? 'text-gray-600' : isInProgress ? 'text-blue-900 font-semibold' : 'text-gray-900'}`}>
                              {getTaskLabel(task)}
                            </p>
                            {isInProgress && (
                            ...
                            {isInProgress && (
                              <span className="text-[10px] text-blue-700 font-medium whitespace-nowrap">Driver en route</span>
                            )}
                          </div>
                          <p className={`text-[11px] truncate ${isCompleted || isSkipped ? 'text-gray-400' : 'text-gray-600'}`}>
                            {getTaskSubtitle(task)}
                          </p>
                        </div>

                        {/* Rich Data Display */}
                        <div className="flex-shrink-0 flex items-center gap-2 justify-end ml-auto">
                          {isSkipped && (() => {
                            const skipReason = getSkipReason(task.task_data);
                            return skipReason ? (
                              <div className="flex items-start gap-1.5 px-2 py-1.5 bg-yellow-50 border border-yellow-200 rounded-lg max-w-xs">
                                <AlertTriangle className="w-3.5 h-3.5 text-yellow-600 mt-0.5 flex-shrink-0" />
                                <div className="text-[11px] text-yellow-800">
                                  <span className="font-medium">Skip reason:</span> {skipReason}
                                </div>
                              </div>
                            ) : null;
                          })()}

                          {task.task_type === 'placement' && isCompleted && !isSkipped && task.photo_url && (
                            <button
                              onClick={() => setFullscreenPhoto(task.photo_url!)}
                              className="relative w-9 h-9 rounded-lg overflow-hidden border-2 border-green-200 hover:border-green-500 transition-colors group flex-shrink-0"
                            >
                            ...
                            <>
                              ...
                              <div className="flex items-center gap-1.5">
                                {task.photo_url && (
                                  <button
                                    onClick={() => setFullscreenPhoto(task.photo_url!)}
                                    className="relative w-9 h-9 rounded-lg overflow-hidden border-2 border-gray-200 hover:border-blue-500 transition-colors group"
                                  >
                                ...
                                {task.after_photo_url && (
                                  <button
                                    ...
                                    className="relative w-9 h-9 rounded-lg overflow-hidden border-2 border-green-200 hover:border-green-500 transition-colors group"
                                  >
