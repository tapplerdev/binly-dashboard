...
// ============================================================================
// HERE MAPS ADDRESS GEOCODING — now a thin delegate
// ============================================================================
//
// These used to call HERE directly from the browser. That had no country filter
// at all, and anchored suggestions at KANSAS CITY, MISSOURI whenever the caller
// passed no location — which all but one caller did. A Toronto tenant typing a
// bin address was ranked against Missouri, and NEXT_PUBLIC_HERE_API_KEY shipped
// in the JS bundle where anyone could read it.
//
// The implementation moved to lib/hooks/use-geocoding.ts, which asks the backend
// so the caller's organization decides the country and the anchor. The names
// below are kept as-is so the ten consuming components need no edit; prefer
// importing from the hook in new code.

import { reverseGeocode as hereReverseGeocode } from '@/lib/hooks/use-geocoding';

export type {
  AddressSuggestion as HereSuggestion,
  ResolvedAddress as HerePlaceDetails,
} from '@/lib/hooks/use-geocoding';

export {
  suggestAddresses as hereAutosuggest,
  lookupAddress as hereLookup,
  reverseGeocode as hereReverseGeocode,
} from '@/lib/hooks/use-geocoding';
